...
export default {
  ...
  computed: {
    ...
    option() {
      ...
      // 偏移量
      var offsetData = []
      const setData = datas.sort(function(a, b) { return a.itemArray[1] - b.itemArray[1] })
      setData.forEach((item, i) => {
        const xSetData = i === 0 ? 0 : i * 120
        const offData = []

        offData.push(xSetData, item.itemArray[1] > 100 ? 100 : item.itemArray[1])
        offsetData.push(offData)
      })
      ...
      const symbolSizeData = datas.map(item => {
        const bol = (80 - 30) / (dataMaxs - dataMins)
        const bolSize = bol * (item.itemArray[1] - dataMins) + 30
        return bolSize
      })
      //
      // 循环定义series的data值

      const datasArr = data.map((item, i) => ({
        ...
        itemStyle: {
          normal: {
            color: new echarts.graphic.RadialGradient(0.3, 0.5, 0.7, [{
              offset: 0,
              color: 'rgb(12, 96, 166,0)'
            },
            {
              offset: 1,
              color: 'rgba(15, 235, 216,0.7)'
            }
            ]),
            opacity: 1,
            shadowColor: '#f7f7f7',
